...
void split(string &s,vector<int> &v){
    string::size_type p2=s.find(','),p1=0;
    while(p2!=string::npos){
        v.push_back(stoi(s.substr(p1,p2-p1)));
        p1=p2+1;p2=s.find(',',p1);
    }
    v.push_back(stoi(s.substr(p1,s.size()-p1)));
}
void SplitString(const std::string& s, std::vector<int>& v, char c){
	std::string::size_type pos1, pos2;
	pos2 = s.find(c);pos1 = 0;v.clear();
	while(std::string::npos != pos2){
		v.push_back(stoi(s.substr(pos1, pos2-pos1)));
    	pos1 = pos2 + 1;pos2 = s.find(c, pos1);
        //cout<<pos1<<" "<<pos2<<"\n";
  	}
